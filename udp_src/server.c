...
int solde(char id_compte[],char message[]){
  ...
  int step2 = sqlite3_step(stmt2);
  if (step2 == SQLITE_ROW) {
    const unsigned char *text = sqlite3_column_text(stmt2, 0); 

    if (text != NULL) {
      strncpy(date_op, (const char *)text, sizeof(date_op) - 1);
      date_op[sizeof(date_op) - 1] = '\0';
    } 
    else{
      strcpy(date_op, "N/A");
    }
...
int main() {
    ...
    while (1) {
        ...
        received = recvfrom(serversock, buffer, BUFFSIZE - 1, 0,
        ...
        if (sendto(serversock, message, strlen(message), 0,
                   (struct sockaddr *)&echoclient, clientlen) < 0) {
            Die("Failed to send message to client");
        }
    }
